...
(function() {
  ...
  var utils = module.exports = {
    ...
    rawString: function rawString(str) {
...
function htmlentities (string) {
    ...
    for (var i = 0; i < entities.length; i++) {
        var symbol = entities[i][0],
            entity = entities[i][1];
        if(string.indexOf(symbol) != -1) string = string.replace(symbol, entity);
        if(string.indexOf('&amp;' + entity.substr(1)) != -1) string = string.replace('&amp;' + entity.substr(1), entity);
    }
